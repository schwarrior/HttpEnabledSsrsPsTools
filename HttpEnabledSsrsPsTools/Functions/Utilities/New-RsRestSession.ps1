...
function New-RsRestSession
{
    ...
    try
    {
        ...
        if ($Credential)
        {
            $result = Invoke-WebRequest -AllowUnencryptedAuthentication -Uri $meUri -Credential $Credential -SessionVariable mySession -UseBasicParsing -Verbose:$false -ErrorAction Stop
        }
        else
        {
            $result = Invoke-WebRequest -AllowUnencryptedAuthentication -Uri $meUri -UseDefaultCredentials -SessionVariable mySession -UseBasicParsing -Verbose:$false -ErrorAction Stop
        }
